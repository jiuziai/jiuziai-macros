...
pub fn check_field_info_boundary(info: &mut MateInfo) -> Result<()> {
    ...
    if let Some(deep) = &info.deep {
        ...
        if is_collection(&strip_option(&info.ty)) {
            // 检查 deep 是否有验证规则
            if !has_any_validation_rules(deep) {
                return Err(Error::new(
                    deep.span,
                    "Collection depth verification must have validation rules inside deep()",
                ));
            }
...
pub fn is_required_type(info: &MateInfo) -> Result<()> {
...
pub fn is_not_empty_type(info: &MateInfo) -> Result<()> {
...
pub fn is_not_blank_type(info: &MateInfo) -> Result<()> {
...
pub fn is_no_space_type(info: &MateInfo) -> Result<()> {
...
pub fn is_size_type(info: &MateInfo) -> Result<()> {
...
/// 检查类型是否支持 regex 检查
/// regex 允许作用于字符串类型
pub fn is_regex_type(info: &MateInfo) -> Result<()> {
...
/// 检查是否有任何验证规则
pub fn has_any_validation_rules(info: &MateInfo) -> bool {
