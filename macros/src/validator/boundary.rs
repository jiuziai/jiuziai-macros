...
pub fn check_field_info_boundary(info: &mut MetaInfo, depth: u8) -> Result<()> {
    check_message_rules(&info)?;
    is_required_type(&info, depth)?;
    is_not_empty_type(&info, depth)?;
    is_not_blank_type(&info, depth)?;
    is_no_space_type(&info, depth)?;
    is_size_type(&info, depth)?;
    is_regex_type(&info, depth)?;

    // 检查 deep 验证
    if let Some(deep) = &info.deep {
        has_deep_not_allowed(deep)?;
        check_message_rules(&deep)?;
        // 这里用父级 MetaInfo 检查是否是集合
        if is_collection(&info.ty) {
            is_required_type(&deep, depth + 1)?;
            is_not_empty_type(&deep, depth + 1)?;
            is_not_blank_type(&deep, depth + 1)?;
            is_no_space_type(&deep, depth + 1)?;
            is_size_type(&deep, depth + 1)?;
            is_regex_type(&deep, depth + 1)?;
        } else {
            // 非集合类型：不允许deep有任何选项
...
pub fn is_required_type(info: &MetaInfo, depth: u8) -> Result<()> {
    if let Some(required) = &info.required {
        if depth < 1 {
            ...
            if is_collection(&info.ty) {
                let element_ty = get_collection_element_type(&info.name, &info.span, &info.ty)?;
...
pub fn is_not_empty_type(info: &MetaInfo, depth: u8) -> Result<()> {
    if let Some(not_empty) = &info.not_empty {
        if depth < 1 {
            ...
            if is_collection(&info.ty) {
                let element_ty = get_collection_element_type(&info.name, &info.span, &info.ty)?;
                if !(is_string_type(&element_ty) || is_collection(&element_ty)) {
                    return Err(Error::new(
                        not_empty.span,
                        "`not_empty` is only allowed to operate on fields of type `collection` or `string`",
                    ));
...
pub fn is_not_blank_type(info: &MetaInfo, depth: u8) -> Result<()> {
...
pub fn is_no_space_type(info: &MetaInfo, depth: u8) -> Result<()> {
...
pub fn is_size_type(info: &MetaInfo, depth: u8) -> Result<()> {
...
/// 检查类型是否支持 regex 检查
/// regex 允许作用于字符串类型
pub fn is_regex_type(info: &MetaInfo, depth: u8) -> Result<()> {
...
/// 获取集合类型的元素类型
/// 获取集合类型的元素类型
pub fn get_collection_element_type(ident: &Ident, span: &Span, ty: &Type) -> Result<Type> {
    let option_element_ty = {
        let ty = strip_option(ty); // 返回 &Type
        if let Type::Path(TypePath { path, .. }) = ty {
            if let Some(seg) = path.segments.last() {
                if let syn::PathArguments::AngleBracketed(args) = &seg.arguments {
                    ...
                    match seg.ident.to_string().as_str() {
                        "Vec" | "VecDeque" | "LinkedList" | "HashSet" | "BTreeSet" | "SmallVec"
                            if type_args.len() == 1 =>
                        {
                            Some(type_args[0].clone()) // 返回克隆的类型
                        }
                        "HashMap" | "BTreeMap" | "IndexMap" if type_args.len() == 2 => {
                            Some(type_args[1].clone()) // 返回克隆的类型
                        }
                        _ => None,
                    }
                } else {
    ...
    match option_element_ty {
        Some(ty) => Ok(ty),
        None => Err(Error::new(
            *span,
            format!("Unable to resolve `{}` element type", ident),
        )),
    }
}
/// helpers: Option<T> -> T
